...
##' @export
fpca_sc <- function(
    data, Y.pred = NULL, argvals = NULL, random.int = FALSE,
  ...
  if (center) {
    if (random.int) {
      ...
      gam0 <- gam(as.vector(data) ~ s(d.vec, k = nbasis))
    }
  ...
  if (cov.est.method == 2) {
    ...
    for (i in 1:I) {
      ...
      cov.count[obs.points, obs.points] <- cov.count[obs.points, obs.points] +
        1
      cov.sum[obs.points, obs.points] <- cov.sum[obs.points, obs.points] + tcrossprod(data.tilde[
        i,
        obs.points
      ])
    }
    ...
    npc.0 <- matrix(predict(gam(G.0.vec ~ te(row.vec, col.vec, k = nbasis)), newdata = data.frame(
      row.vec = row.vec.pred,
  ...
  if (makePD) {
    npc.0 <- {
      tmp <- Matrix::nearPD(npc.0,
        corr = FALSE, keepDiag = FALSE, do2eigen = TRUE,
        trace = TRUE
      )
      as.matrix(tmp$mat)
    }
  ...
  ret <- lapply(seq_along(ret.objects), function(u) get(ret.objects[u]))
