...
fpca_ssvd <- function(
    Y = NULL, argvals = NULL, npc = NA, center = TRUE, maxiter = 15,
  ...
  if (!is.null(argvals)) {
    ...
    if (any(diff(argvals) / mean(diff(argvals)) > 1.05 |
      ...
      warning(paste(
        "non-equidistant <argvals>-grid detected:",
  ...
  for (k in 1:npc) {
    ...
    while (all(c(reldiff > tol, iter <= maxiter))) {
      ...
      if (gridsearch) {
        ...
        minalpha <- optimize(
          f = gcv, interval = c(lower.alpha, upper.alpha),
  ...
  if (length(uhoh)) {
    warning(
      "First SV for remaining un-smooth signal larger than ",
